...
public class ExampleVDHT {
    ...
    private static void exampleDHT(final PeerDHT[] peers, Number160 nr)
        ...
        FuturePut fp = peers[0].put(Number160.ONE).data(new Data("start -")).versionKey(Number160.ZERO)
        ...
        FutureRemove fr = peers[5].remove(Number160.ONE).versionKey(Number160.ONE).start().awaitUninterruptibly();
        ...
        for (Object object : fg.rawData().values().iterator().next().values().toArray()){
            Data data = (Data)object;

            System.out.println("Data: " + data.object().toString());
        }

        for (Number640 num : fg.rawData().values().iterator().next().keySet()){
            System.out.println("Version: " + num.versionKey().toString());
        }
    }
    ...
    private static void storeDHT(PeerDHT peerDHT, String string)
        ...
        synchronized (locker) {
            Number160 version = new Number160(vers);//Number160.createHash(UUID.randomUUID().toString());
            vers++;
            System.out.println("Version generated: " + version.toString() + " for: " + string);

            FuturePut fp = peerDHT.put(Number160.ONE).versionKey(version).data(new Data(fg.data().object() + string)).start()
                    .awaitUninterruptibly();
    ...
    private static Pair<Number160, Data> getAndUpdate(PeerDHT peerDHT, String string)
