...
//        tx.setChallenge(challenge);
    ...
    public boolean verifyTransaction(Transaction transaction) {
        ...
        Transaction transactionLocal = new Transaction(transaction);

        transactionLocal.setSignature(null);
        transactionLocal.setChallenge(null);

        byte[] message = serializationService.getHash(transactionLocal);
    ...
    public List<Transaction> getTransactions(Blockchain blockchain, Block block) throws IOException, ClassNotFoundException {
        ...
        if (transactions.size() != hashes.size()) {
            transactions = AppServiceProvider.getBlockchainService().get(blockHash, blockchain, BlockchainUnitType.BLOCK_TRANSACTIONS, false);
        }
